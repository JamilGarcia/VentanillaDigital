...
import { getCategorias, getInstituciones, getTramites } from '../services/api';
import { LayoutGrid, List, Layers, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
...
export default function Catalog() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [cats, insts, trams] = await Promise.all([
          getCategorias(),
          getInstituciones(),
          getTramites()
        ]);
  ...
  // Sincronizar estado cuando cambian los search params
  useEffect(() => {
  ...
  const filteredTramites = tramites.filter(t => {
    ...
    const matchesCategory = selectedCategory === 'all' || t.categoria_id === selectedCategory || t.categoriaId === selectedCategory;
    const matchesInst = selectedInst === 'all' || t.institucion_id === selectedInst || t.institucionId === selectedInst;
